...
Here I register some extra links for org-mode so that I can link to outlook files.
...
It is also worth noting that adding some VBA Macros to outlook can be useful. 

First one macro to create new tasks with a link back to the email.

#+BEGIN_SRC visual-basic
'Adds a task to an org file. (/!\ Add reference to Microsoft Scritping Runtime)
Sub AddTask()

   Dim objMail As Outlook.MailItem
   Dim fso As FileSystemObject
   Dim orgfile As TextStream
   ...
   Set orgfile = fso.OpenTextFile("c:\ericst\org\outlook.org", ForAppending, True)
   
   orgfile.WriteLine "* TODO Email: [[outlook:" + objMail.EntryID + "][" + objMail.Subject + " (" + objMail.SenderName + ")]]"
   orgfile.Close

End Sub
#+END_SRC

We can also have a macro that copies a link to the clipboard so that you insert it wherever you want.
#+BEGIN_SRC visual-basic
'Adds a link to the currently selected message to the clipboard (/!\ Add reference to Microsoft Forms 2.0 Object Library)
Sub CopyLink()
   ...
   Set objMail = Application.ActiveExplorer.Selection.Item(1)
   doClipboard.SetText "[[outlook:" + objMail.EntryID + "][" + objMail.Subject + " (" + objMail.SenderName + ")]]"
   doClipboard.PutInClipboard
   
End Sub
#+END_SRC

Last, when e-mailing notes, it is interesting to actually be able to insert html in your mail. 
Here is a macro that do just that.

#+BEGIN_SRC visual-basic
'Prepends the content of the clipboard to the message (/!\ Add reference to Microsoft Forms 2.0 Object Library)
Sub PrependHTML()
...
It is important for those macros to work that you add the following referrences:

- Microsoft Forms 2.0 Object Library \\
  For clipboard manipulations
- Microsoft Scritping Runtime \\
  For the text file manipulations
* Spellchecking
...
* Start the sever
This is necessary on windows as you cannot run an emacs service.
#+begin_src emacs-lisp :tangle space-uconfig.el
(server-start)
#+end_src
* Trailing whitespaces
Deactivate the highlight of trailing whitespace.
#+begin_src emacs-lisp :tangle space-uconfig.el
