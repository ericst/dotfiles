...
Memex stands for memory extender. The idea is to collect knowledge in a sort of
all encompassing wiki. Org-roam is system that allows to easily manage
collection of org-file by also keeping track of back links. 
...
Of course, we need that system to be included into our work environment.
