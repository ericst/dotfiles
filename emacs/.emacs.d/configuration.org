...
- [X] Basic functionality [100%]
  - [X] Project management with ~projectile~
  - [X] Setup ~ivy~
  - [X] Jumps with ~avy~
  - [X] Take a look at ~dumb-jump~
  - [X] Configure ~flyspell~
- [X] Fix autosave [[* Backup files and autosave]]
- [ ] Start better key binding description
- [X] Org-mode [100%]
  - [X] Org-mode UI
  - [X] Org-mode System
  - [X] Org-mode Capture
  - [X] Org-mode Daily Logs
- [X] Clean up hooks in [[* Lisp & Schemes]]
...
* Org-mode
Some general notes on my system. Inspired by [[https://fortelabs.co/blog/para/][PARA]]
- ~~exocortex/projects/~ :: Contains projects. Which is collection of tasks with a common, time-delimited goal.
- ~~exocortex/spheres/~ :: Contains spheres of {influence, responsibilities}. Which are areas, spheres or interest where a standard to be maintained over time.
- ~~exocortex/logs.org~ :: Contains logs, journal of events. Mainly as a record keeping mechanism.
- ~~exocortex/notes/~ :: Contains permanent notes on topics. At the end it should look like the described method in this book [fn:1].
- ~~exocortex/archives/~ :: When projects are finished, or spheres become obsolete, I move them here.
...
This is the capture configuration. 
Basically, I have only two basic templates:
...
#+begin_src emacs-lisp
  ...
  (setq org-capture-templates
        '(("t" "Task" entry (file+olp "~/exocortex/spheres/scheduler.org" "Inbox")
           "* TASK %?\n %i\n")
